        ...
        <div class="col-lg-7">
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-white border-bottom py-3">
                    <h6 class="mb-0 fw-semibold"><i class="bi bi-pencil me-2 text-primary"></i>Category Details</h6>
                </div>
                    ...
                    <form action="{{ route('admin.categories.update', $category->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <small class="text-muted">
                                <i class="bi bi-link-45deg"></i> Slug: <code>{{ $category->slug }}</code>
                                &mdash; سيتغير تلقائياً لو غيرت الـ Title
                            </small>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="summary"
                                      ...
                                      rows="3"
                                      placeholder="Short description...">{{ old('summary', $category->summary) }}</textarea>
                            @error('summary')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="is_parent"
                                    ...
                                    id="isParentSelect">
                                <option value="1" {{ old('is_parent', $category->is_parent) == '1' ? 'selected' : '' }}>Parent Category</option>
                                <option value="0" {{ old('is_parent', $category->is_parent) == '0' ? 'selected' : '' }}>Sub Category</option>
                            </select>
                            @error('is_parent')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3" id="parentField"
                             style="{{ old('is_parent', $category->is_parent) == '0' ? '' : 'display:none' }}">
...
<script>
    document.getElementById('isParentSelect').addEventListener('change', function () {
        document.getElementById('parentField').style.display = this.value === '0' ? 'block' : 'none';
    });
</script>
@endsection
